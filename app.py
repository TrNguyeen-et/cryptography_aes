...
# Helper chuyển mảng 16 phần tử thành ma trận 4x4 theo CỘT (Column-Major)
def _to_4x4_matrix(arr):
    return [
...
def _collect_encrypt_steps(plaintext_block: bytes, key: bytes) -> dict:
    ...
    rounds.append({'round': 0, 'label': 'Initial AddRoundKey', 'steps': [{'name': 'AddRoundKey', 'state': state[:], 'old_state': old_state[:], 'key': round_keys[0][:], 'details': details}]})
    ...
    for r in range(1, Nr):
        ...
        mat_before = _to_4x4_matrix(old_state)
        mat_after = _to_4x4_matrix(state)
        details = [{"row": 0, "desc": "Không dịch", "before": mat_before[0], "after": mat_after[0]},
                   {"row": 1, "desc": "Dịch trái 1", "before": mat_before[1], "after": mat_after[1]},
                   {"row": 2, "desc": "Dịch trái 2", "before": mat_before[2], "after": mat_after[2]},
                   {"row": 3, "desc": "Dịch trái 3", "before": mat_before[3], "after": mat_after[3]}]
...
def _collect_decrypt_steps(ciphertext_block: bytes, key: bytes) -> dict:
    ...
    for r in range(Nr - 1, 0, -1):
        ...
        mat_after = _to_4x4_matrix(state)
        details = [{"row": 0, "desc": "Không dịch", "before": mat_before[0], "after": mat_after[0]},
                   {"row": 1, "desc": "Dịch phải 1", "before": mat_before[1], "after": mat_after[1]},
                   {"row": 2, "desc": "Dịch phải 2", "before": mat_before[2], "after": mat_after[2]},
                   {"row": 3, "desc": "Dịch phải 3", "before": mat_before[3], "after": mat_after[3]}]
