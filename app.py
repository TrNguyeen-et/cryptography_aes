...
def api_encrypt():
    ...
    try:
        ...
        if mode == 'ECB':
            ...
            if verbose and len(pt) <= 16: result['verbose'] = _collect_encrypt_steps(pkcs7_pad(pt)[:16], key)
        elif mode == 'CBC':
            ...
            if verbose and len(pt) <= 16: result['verbose'] = _collect_encrypt_steps(bytes([p ^ i for p, i in zip(pkcs7_pad(pt)[:16], iv)]), key)
...
def api_decrypt():
    ...
    try:
        ...
        if mode == 'ECB':
            ...
            if verbose and len(ct) == 16: result['verbose'] = _collect_decrypt_steps(ct, key)
        elif mode == 'CBC':
