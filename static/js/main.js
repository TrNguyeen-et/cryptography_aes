...
function renderDetailTable(stepData) {
  ...
  if (name.includes('SubBytes') || name.includes('InvSubBytes')) {
    ...
    // Thêm bảng S-Box bên phải
    wrapper.appendChild(renderSboxTable(inputs));
    ...
    details.forEach(d => { html += `<tr><td>Cột ${d.col}</td><td>${d.input.map(b => byteToHex(b)).join(' ')}</td><td class="highlight">${d.output.map(b => byteToHex(b)).join(' ')}</td></tr>`; });
    tableContainer.innerHTML = html + '</table>'; wrapper.appendChild(tableContainer);
