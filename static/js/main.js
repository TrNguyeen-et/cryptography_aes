...
function asciiToHex(str) { try { const utf8Bytes = new TextEncoder().encode(str); return Array.from(utf8Bytes).map(b => b.toString(16).padStart(2, '0')).join(''); } catch (e) { return ''; } }
...
// Render Ma trận State 4x4 (Lấy thẳng từ kết quả backend trả về)
function renderStateGrid(stateArr) {
  const grid = document.createElement('div'); grid.className = 'state-grid';
  // Backend trả về mảng 16 phần tử. AES lưu theo cột: [0,1,2,3] là cột 0.
  // Muốn hiển thị đúng dạng ma trận 4x4 trên lưới, ta map theo thứ tự:
  // Hàng 0: 0, 4, 8, 12
  // Hàng 1: 1, 5, 9, 13
  // Hàng 2: 2, 6, 10, 14
  // Hàng 3: 3, 7, 11, 15
  const displayOrder = [0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15];
  for (let i = 0; i < 16; i++) { 
    const cell = document.createElement('div'); 
    cell.className = 'state-cell'; 
    cell.textContent = byteToHex(stateArr[displayOrder[i]]); 
    grid.appendChild(cell); 
  }
  return grid;
}

// Render bảng S-Box (Highlight ô tại tọa độ = giá trị đầu vào)
function renderSboxTable(inputs, isInverse = false) {
  ...
  for (let row = 0; row < 16; row++) {
    ...
    for (let col = 0; col < 16; col++) {
      const inputVal = row * 16 + col; // Tọa độ = Đầu vào
      const outputVal = tableData[inputVal]; // Giá trị trong ô = Đầu ra
      
      const cell = document.createElement('div'); cell.className = 'sbox-cell'; cell.textContent = byteToHex(outputVal);
      if (inputs.includes(inputVal)) cell.classList.add('sbox-highlight');
      grid.appendChild(cell);
    }
  }
...
function renderKeyScheduleSteps(data) {
  ...
  data.round_keys.forEach((rk, idx) => {
    ...
    for (let i = startIdx; i < endIdx; i++) {
      ...
      if (i < data.Nk) {
        const w_orig = rk.bytes.slice((i % 4) * 4, (i % 4) * 4 + 4);
        stepDiv.innerHTML = `
          <div class="ks-step-title">W[${i}] = Khóa gốc (Byte ${i*4} đến ${i*4+3})</div>
          <div class="ks-step-row">
            <span class="ks-label">W[${i}]:</span> <span class="ks-val highlight">${w_orig.map(b=>byteToHex(b)).join(' ')}</span>
          </div>
        `;
      } else {
        const s = data.steps[i - data.Nk];
        
        if (s.type === 'rcon') {
          stepDiv.innerHTML = `
            <div class="ks-step-title">W[${s.i}] = W[${s.i-s.Nk}] ⊕ g(W[${s.i-1}])</div>
            <div class="ks-step-row"><span class="ks-label">W[${s.i-1}]:</span> <span class="ks-val">${s.w_im1.map(b=>byteToHex(b)).join(' ')}</span></div>
            <div class="ks-step-row"><span class="ks-label">1. RotWord:</span> <span class="ks-val">${s.rotated.map(b=>byteToHex(b)).join(' ')}</span></div>
            <div class="ks-step-row"><span class="ks-label">2. SubWord:</span> <span class="ks-val">${s.subbed.map(b=>byteToHex(b)).join(' ')}</span></div>
            <div class="ks-step-row"><span class="ks-label">3. ⊕ Rcon[${s.i/s.Nk}]:</span> <span class="ks-val">${s.rcon.map(b=>byteToHex(b)).join(' ')}</span></div>
            <div class="ks-step-row"><span class="ks-label">= g(W[${s.i-1}]):</span> <span class="ks-val highlight">${s.xored_rcon.map(b=>byteToHex(b)).join(' ')}</span></div>
          ...
          stepDiv.innerHTML = `
            ...
            <div class="ks-step-row"><span class="ks-label">W[${s.i-1}]:</span> <span class="ks-val">${s.w_im1.map(b=>byteToHex(b)).join(' ')}</span></div>
            <div class="ks-step-row"><span class="ks-label">SubWord:</span> <span class="ks-val">${s.subbed.map(b=>byteToHex(b)).join(' ')}</span></div>
            <div class="ks-step-row" style="border-top:1px solid var(--border);margin-top:4px;padding-top:4px;">
              <span class="ks-label">W[${s.i-s.Nk}]:</span> <span class="ks-val">${s.w_iNk.map(b=>byteToHex(b)).join(' ')}</span>
              <span class="ks-op">⊕</span>
...
async function doEncrypt() {
  ...
  try {
    ...
    if (mode === 'CBC' && data.iv_hex) {
      ...
      if (data.iv_generated) showToast('⚠️ IV đã tự sinh ngẫu nhiên! Hãy copy lại để giải mã.', 4000);
    } else { ivRow.style.display = 'none'; }
